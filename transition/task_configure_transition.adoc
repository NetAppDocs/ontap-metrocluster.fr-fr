...
+
.. Joignez le premier nœud IP MetroCluster (node_A_1-IP) à la configuration FC MetroCluster existante.
+
....
...
.
....
.. Associez le second nœud IP MetroCluster (node_A_2-IP) à la configuration FC MetroCluster existante.


. Répétez ces étapes pour joindre le noeud_B_1-IP et le noeud_B_2-IP au cluster_B.
...
NOTE: Vous pouvez configurer les interfaces IP MetroCluster depuis n'importe quel cluster. En outre, à partir de ONTAP 9.9.1, si vous utilisez une configuration de couche 3, vous devez également spécifier le `-gateway` Paramètre lors de la création des interfaces IP MetroCluster. Reportez-vous au lien :../install-ip/concept_considérations_layer_3.html.
...
.Étapes
. Configurer le MetroCluster pour implémenter les modifications :
+
