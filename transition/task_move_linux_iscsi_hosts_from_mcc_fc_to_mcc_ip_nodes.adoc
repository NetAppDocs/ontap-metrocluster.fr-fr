...
----
. Lister les interfaces iSCSI en ONTAP pour les SVM contenant les interfaces : `iscsi interface show -vserver svm-name`
+
[listing]
----
...
----
. Vérifiez que le `sg3-utils` Le package est installé sur l'hôte Linux. Cela permet d'éviter un `rescan-scsi-bus.sh utility not found` Erreur lors de la nouvelle analyse de l'hôte Linux pour les LUN nouvellement mappées à l'aide du `rescan-scsi-bus` commande.
. Sur l'hôte iSCSI, lancez une nouvelle analyse du bus SCSI : `/usr/bin/rescan-scsi-bus.sh -r`
+
...
.
----
. Sur l'hôte iSCSI, vérifiez que seuls les chemins depuis les nœuds IP MetroCluster sont visibles :
+
`sanlun lun show -p`
