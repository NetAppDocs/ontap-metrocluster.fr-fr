...
---
= Déplacez les hôtes iSCSI Linux des nœuds FC MetroCluster vers les nœuds IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Les nœuds FC MetroCluster sont appelés anciens nœuds et les nœuds IP MetroCluster sont appelés nouveaux nœuds.
...
Les LUN et les volumes sont toujours physiquement hébergés sur les anciens nœuds. Comme les LUN ne sont signalés que sur les interfaces des anciens nœuds, l'hôte n'affiche que les chemins passant par les anciens nœuds. Pour le constater, exécutez les commandes  `sanlun lun show -p` et  `multipath -ll -d` sur l'hôte et examinez les résultats des commandes.
