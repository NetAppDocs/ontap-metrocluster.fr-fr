...
Vous pouvez tester une opération de basculement négociée (planifiée) pour confirmer la disponibilité des données sans interruption.
...
----
. Vérifier que tous les agrégats sont bien `mirrored`:
+
...
. Surveillez le résultat sur la console à l'aide du `system environment sensors show -state fault` et `storage shelf show -errors` commandes.
+
[listing]
----
