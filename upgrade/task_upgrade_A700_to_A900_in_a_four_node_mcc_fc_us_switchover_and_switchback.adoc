...
[[remove-the-aff-a700-or-fas9000-controller-module-and-nvs-at-siteb]]
...
[[remove-the-controller-module-and-nvs-from-both-nodes-at-siteb]]
...
+
L'état HA de tous les composants doit être `mcc`.
...
. Sur chaque nœud, vérifiez la date, l'heure et le fuseau horaire du système :
+
`show date`

. Si nécessaire, définissez la date en UTC ou en temps moyen de Greenwich (GMT) :
+
`set date <mm/dd/yyyy>`

. Vérifiez l'heure en utilisant la commande suivante à l'invite de l'environnement de démarrage :
+
`show time`

. Si nécessaire, réglez l'heure en UTC ou GMT :
+
`set time <hh:mm:ss>`
...
. Rassemblez les variables d'environnement :
+
`printenv`
...
. Suivez les étapes décrites dans : link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Ajout des configurations MetroCluster"] dans la section _Installation et configuration du système de départage MetroCluster_.
