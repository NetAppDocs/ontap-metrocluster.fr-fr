...
. Mettre à jour les commutateurs de la structure B (commutateurs 2 et 4) en répétant ,Étape 2 à ,Étape 5.
...
+
Au cours de la procédure de remplacement, vous remplaceront ces ID système par les ID système des modules de contrôleur.
...
NOTE: Si des modules d'extension sont utilisés comme dispositifs de vidage de mémoire sur les modules de stockage non volatile des systèmes AFF A700 ou FAS9000, ne transférez pas ces modules vers les NVS AFF A900 ou FAS9500. Ne transférez aucune pièce du module de contrôleur AFF A700 ou A900 et NVS vers le module de contrôleur AFF A900 ou FAS9500.
...
`netboot http://<web_server_ip/path_to_web_accessible_directory>/netboot/kernel`Le `<path_to_the_web-accessible_directory>` vous devez indiquer où vous avez téléchargé le `<ontap_version>_image.tgz` dans ,Étape 2.
+
...
. Si vous êtes invité à poursuivre la procédure, entrez `y`, Et lorsque vous êtes invité à saisir l'URL du pack :http://<web_server_ip/path_to_web-accessible_directory>/<ontap_version>_image.tgz[]
