...
* Le tableau présente tous les modèles de plateforme qui utilisent des *ports MetroCluster/HA partagés* et des *ports de cluster/HA partagés*. Toutes les combinaisons ne sont pas prises en charge par cette procédure. Reportez-vous à la link:concept_choosing_tech_refresh_mcc.html#supported-metrocluster-ip-tech-refresh-combinations["combinaisons de plateformes prises en charge"] pour vérifier si votre combinaison d'actualisation est prise en charge.
...
Disjoncteur d'attache
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Ajout des configurations MetroCluster"] dans _Installation et configuration du bris d'égalité MetroCluster_.
