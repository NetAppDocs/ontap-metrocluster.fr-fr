...
* Le tableau présente tous les modèles de plateforme qui utilisent des *ports MetroCluster/HA partagés* et des *ports de cluster/HA partagés*. Toutes les combinaisons ne sont pas prises en charge par cette procédure. Reportez-vous à la <<expand-ip-supported-platforms,combinaisons de plateformes prises en charge>> pour vérifier si votre combinaison d’extension est prise en charge.
...
[[expand-ip-supported-platforms]]
== Combinaisons de plateformes prises en charge lors de l'ajout d'un deuxième groupe de reprise après incident

Les tableaux suivants présentent les combinaisons de plateformes prises en charge pour les configurations IP à huit nœuds MetroCluster.
...
[[remove-the-configuration-from-monitoring-applications]]
...
[[prepare-the-new-controller-modules]]
...
[[configure-intercluster-lifs-for-cluster-peering]]
...
+
Pour connaître la syntaxe complète des commandes, consultez le link:https://docs.netapp.com/us-en/ontap-cli["Référence des commandes ONTAP"^].
...
+
L'exemple suivant attribue les ports « e0e » et « e0f » au groupe de basculement « intercluster01 » sur la SVM « cluster01 » :
...
+
L'exemple suivant montre que les LIF intercluster « cluster01_icl01 » et « cluster01_icl02 » sur le port « e0e » basculeront vers le port « e0f ».
...
-------- --------------- --------------------- --------------- --------
cluster01
         cluster01_icl01 cluster01-01:e0c   local-only         Default
                            Failover Targets: cluster01-01:e0c,
                                              cluster01-01:e0d
         cluster01_icl02 cluster01-02:e0c   local-only         Default
...
[[create-the-metrocluster-interfaces-and-mirror-root-aggregates]]
...
+
`cluster peer modify -cluster cluster_B -peer-addrs <node_B_1-old IC port_ip_address>,<node_B_2-old IC port_ip_address>,<node_B_1-new IC port_ip_address>,<node_B_2-new IC port_ip_address>`

+
Depuis le cluster_B, exécutez la commande suivante :
...
NOTE: Le résultat de la  `metrocluster configuration-settings connection show` commande peut varier selon votre modèle de plateforme. Par exemple, sur certaines plateformes, les lignes pour HA Partner ne sont pas affichées dans le résultat.
...
[[finalize-the-addition-of-the-new-nodes]]
