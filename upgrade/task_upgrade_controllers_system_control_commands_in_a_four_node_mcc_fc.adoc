...
* Cette procédure vous permet de mettre à niveau les contrôleurs d'une configuration FC MetroCluster à quatre nœuds à l'aide du basculement et du rétablissement automatisés basé sur NSO. Pour effectuer une mise à niveau du contrôleur à l'aide du transfert d'agrégats (ARL), reportez-vous à https://["Utilisez les commandes « system contrôleur replace » pour mettre à niveau le matériel du contrôleur exécutant ONTAP 9.8 ou une version ultérieure"]. Il est recommandé d'utiliser la procédure automatisée basée sur le système NSO.
...
.Étapes
. http://["Supprimez la configuration MetroCluster existante"^] À partir du logiciel disjoncteur d'attache.
...
. Suivez les étapes de la section http://["Ajout des configurations MetroCluster"].
