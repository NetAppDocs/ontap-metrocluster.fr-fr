...
* Vous pouvez utiliser cette procédure pour mettre à niveau les contrôleurs dans une configuration MetroCluster FC à quatre nœuds à l'aide du basculement et du retour automatiques basés sur NSO. Si vous souhaitez effectuer une mise à niveau des contrôleurs à l'aide de la relocalisation d'agrégats (ARL), consultez link:https://docs.netapp.com/us-en/ontap-systems-upgrade/upgrade-arl-auto-app/["Utilisez les commandes « system contrôleur replace » pour mettre à niveau le matériel du contrôleur exécutant ONTAP 9.8 ou une version ultérieure"^]. Il est recommandé d'utiliser la procédure automatisée basée sur NSO.
