...
image::../media/workflow_ip_upgrade.png[Processus de mise à niveau des contrôleurs dans la configuration IP de MetroCluster]



[[prepare-for-the-upgrade]]
[[upgrade_a700_a900_ip_prepare]]
...
. Si les nœuds MetroCluster sont configurés avec un gestionnaire de clés externe pour la SVM d'administration (cluster), copiez les informations du serveur de clés dans un emplacement sécurisé. Si le gestionnaire de clés intégré est configuré, copiez les informations de sauvegarde et la phrase secrète dans un emplacement sécurisé.
+
[role="tabbed-block"]
====
.Gestionnaire de clés embarqué
--
.. Exécutez la commande suivante pour afficher la sauvegarde du Onboard Key Manager :
+
`security key-manager onboard show-backup`

+
Vous aurez également besoin de la phrase secrète la plus récente du Onboard Key Manager plus tard dans la procédure de mise à niveau.
...
--
.Gestionnaire de clés externes
--
Rassemblez les informations de gestion des clés externes dont vous aurez besoin pour restaurer la configuration du gestionnaire de clés ultérieurement dans la procédure.

.. Afficher le nom du SVM d'administration (cluster).
+
`vserver show -type admin`

+
Dans cet exemple, le nom de la SVM d'administration est cluster_B :

+
[listing]
----
cluster_B::*> vserver show -type admin
...
cluster_B
            admin   -          -          -           -          -
----
.. Obtenez l'adresse et le numéro de port d'un serveur de clé primaire associé au gestionnaire de clés externe configuré sur le SVM d'administration (cluster) :
+
[NOTE]
====
*** Le SVM d'administration peut avoir jusqu'à quatre serveurs de clés primaires, mais vous n'aurez besoin que du numéro de port et de l'adresse de l'un des serveurs de clés primaires pour restaurer la configuration de gestion des clés plus tard dans la procédure.
*** Si un nom d'utilisateur et un mot de passe sont associés au serveur de clé primaire que vous choisissez, vous aurez également besoin de ces informations pour le processus de récupération.
...
----
.. Si vous utilisez ONTAP 9.16.1 ou une version ultérieure, obtenez l'UUID du keystore pour la SVM d'administration.
+
`security key-manager show-key-store -vserver <admin_svm_name> -fields uuid`

+
[listing]
----
cluster_B::*>  security key-manager show-key-store -vserver cluster_B -fields uuid
vserver         uuid
--------------- ------------------------------------
cluster_B        f281aa55-d635-11f0-93bc-005056bb68f2

cluster_B::*
----
.. Enregistrez les informations recueillies dans un endroit sûr pour une utilisation ultérieure dans la procédure. Vous devriez maintenant avoir recueilli les informations suivantes :
+
*** Le numéro de port et l'adresse de l'un des serveurs de clés primaires pour la SVM d'administration
*** Le nom d'utilisateur et le mot de passe si l'un est associé au serveur de clé primaire choisi
*** L'UUID du magasin de clés pour la SVM d'administration (requis dans ONTAP 9.16.1 ou version ultérieure)
...
Disjoncteur d'attache
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#remove-metrocluster-configurations["Suppression des configurations MetroCluster"] dans le contenu _Installation et configuration de MetroCluster Tiebreaker_
...
image::../media/mcc_upgrade_cluster_a_in_switchover_A900.png[Site_B inactif et prêt à être mis à niveau après le basculement de MetroCluster]
...
+
image::../media/drw_9500_remove_PCM.png[Retrait du module de commande du châssis : déverrouiller et faire glisser le module vers l'extérieur]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[Retrait du module NVRAM du châssis : déverrouiller et tirer les languettes]
...
+
image::../media/drw_a900_move-remove_NVRAM_module.png[Installation du module NVRAM dans le châssis : aligner et verrouiller en place]
...
+
image::../media/drw_9500_remove_PCM.png[Installation du module de commande dans le châssis : aligner, placer et câbler]
...
.Étapes
. Préparez les commutateurs IP pour l'application des nouveaux fichiers de configuration de référence (RCF).
+
Suivez les étapes de la section pour votre fournisseur de commutateurs :
...
. Si le SVM d'administration du système MetroCluster est configuré avec un gestionnaire de clés, sélectionnez l'option de menu de démarrage appropriée à votre configuration de gestion des clés.
+
|===
...
Suivez les instructions pour fournir les informations suivantes afin de récupérer et de restaurer la configuration du gestionnaire de clés :

** Sauvegarde et phrase secrète du gestionnaire de clés intégré
...
*Remarque :* Bien que le SVM d’administration puisse avoir jusqu’à quatre serveurs de clés primaires, seules les informations d’un seul serveur de clés primaires peuvent être spécifiées lors du processus de récupération.
...
. Si le chiffrement du volume racine est utilisé, sélectionnez à nouveau l'option du menu de démarrage pour votre configuration de gestion des clés.
+
|===
...
NOTE: Lorsque vous exécutez l’option  `6` du menu de démarrage, les valeurs de bootarg MetroCluster peuvent devoir être réinitialisées. Reportez-vous à link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["Définissez les variables bootarg IP MetroCluster"].
...
image::../media/mcc_upgrade_cluster_a_switchback_A900.png[Site_A en attente d'une mise à niveau après le basculement de MetroCluster]
...
Disjoncteur d'attache
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Ajout des configurations MetroCluster"] dans la section _Installation et configuration du système de départage MetroCluster_.
...
Médiateur
 a| 
link:../install-ip/concept_mediator_requirements.html["Configurer ONTAP Mediator à partir d'une configuration IP MetroCluster"] dans la section _Installation et configuration IP MetroCluster_.
