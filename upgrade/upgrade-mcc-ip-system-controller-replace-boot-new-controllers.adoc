...
+
Utilisez les étapes dans link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Définissez les variables bootarg IP MetroCluster"].

. Vérifiez que le partner-sysid est correct :
+
`printenv partner-sysid`
...
NOTE: Lorsque vous exécutez l’option du menu de démarrage `6`, les valeurs de bootarg de MetroCluster peuvent devoir être réinitialisées. Reportez-vous à link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Définissez les variables bootarg IP MetroCluster"].
...
.. Une fois les LIF inter-clusters opérationnels, vérifiez l'état des entre cluster et rétablissez le peering de cluster si nécessaire.
+
