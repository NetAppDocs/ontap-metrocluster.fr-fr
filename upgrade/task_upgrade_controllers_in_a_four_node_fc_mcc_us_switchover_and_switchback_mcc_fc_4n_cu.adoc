...
. Si le chiffrement du volume root est utilisé, en fonction de la version d'ONTAP que vous utilisez, sélectionnez l'option du menu de démarrage ou exécutez à nouveau la commande du menu de démarrage pour votre configuration de gestion des clés.
+
[role="tabbed-block"]
====
.ONTAP 9.8 et versions ultérieures
