...
Vérifiez que les LIF sont hébergées sur les nœuds et ports appropriés avant d'effectuer un rétablissement.

.Étapes
. Vérifiez que les LIF sont hébergées sur les nœuds et ports appropriés avant d’effectuer un basculement.
+
...
CAUTION: Vous devez vérifier que l'emplacement des LIF de données est correct sur les nouveaux nœuds avant d'effectuer un rétablissement. Lorsque vous rétablissez la configuration, ONTAP tente de reprendre le trafic sur le port d'attache utilisé par les LIF. Une défaillance d'E/S peut survenir lorsque la connexion du port d'attache au port du commutateur et au VLAN est incorrecte. Si vous rencontrez des problèmes pour placer les LIF sur les nœuds ou ports appropriés, n'effectuez pas le rétablissement et contactez le support technique de NetApp.
...
|===
. Effectuez les vérifications suivantes pour confirmer que le basculement entre les deux clusters est opérationnel :
+
.. À partir du cluster de reprise après sinistre, vérifiez si un basculement du cluster survivant vers le cluster de reprise après sinistre est possible :
+
`metrocluster switchover -simulate`

.. À partir du cluster survivant, vérifiez si un basculement du cluster de reprise après sinistre vers le cluster survivant est possible :
+
`metrocluster switchover -simulate`
...
NOTE: Dans un environnement FC MetroCluster, les « connexions » doivent apparaître comme « non applicable » dans la sortie de la commande.
