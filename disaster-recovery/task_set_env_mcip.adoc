...
+
Le résultat de la commande varie en fonction du modèle de plateforme.

+
[role="tabbed-block"]
====
.Systèmes introduits dans ONTAP 9.15.1 ou version ultérieure
--
Les systèmes introduits dans ONTAP 9.15.1 ou version ultérieure (AFF A70, AFF A90 et AFF A1K) possèdent des ports distincts pour la haute disponibilité et la reprise après incident, comme illustré dans l'exemple de sortie suivant :
...
. À l'invite DU CHARGEUR pour chacun des nœuds du site d'incident, définissez la valeur de démarrage en fonction du modèle de plateforme :
+
[NOTE]
====
** Si les interfaces utilisent les VLAN par défaut ou si le modèle de plate-forme n'utilise pas d'ID VLAN (voir la liste ci-dessus), l'ID _vlan_ n'est pas nécessaire.
...
.Systèmes introduits dans ONTAP 9.15.1 ou version ultérieure
--
La valeur de _HA-Partner-IP-address_ doit être définie sur *0* (zéro) sur les systèmes introduits dans ONTAP 9.15.1 ou version ultérieure car ils ont des ports distincts pour DR et HA.
