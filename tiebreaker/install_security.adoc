...
+
** Le chiffrement des données au repos est pris en charge.link:https://mariadb.com/docs/server/security/encryption/data-at-rest-encryption/data-at-rest-encryption-tde-fundamentals["En savoir plus sur le chiffrement des données au repos"^]
...
+
link:https://mariadb.com/docs/server/security/securing-mariadb["Sécuriser MariaDB"^]
...
.Et la suite ?
link:install-choose-procedure.html["Choisissez votre procédure d'installation"]installer ou mettre à jour Tiebreaker.
