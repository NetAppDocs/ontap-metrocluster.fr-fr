...
== Désactiver le mode observateur de départage

Vous pouvez désactiver le mode observateur pour que le logiciel Tiebreaker déclenche automatiquement un basculement après la détection d'une panne de site.


NOTE: Dans Tiebreaker 1.6 et versions ultérieures, vous pouvez activer la commutation automatique lorsque vous ajoutez un moniteur à l'aide de `monitor add wizard` commande. Se référer àlink:concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Ajouter des configurations MetroCluster"] .

.Étape
. Désactiver le mode observateur :
+
