...
Exemple utilisé dans cette procédure : site_A
 a| 
...
Exemple utilisé dans cette procédure : Controller_A_1
 a| 
 a| 
 a| 
 a| 
...
Exemple utilisé dans cette procédure : Controller_A_2
 a| 
 a| 
 a| 
 a| 
...
Non requis pour les configurations MetroCluster à deux nœuds (un nœud sur chaque site).
 ...
 a| 
Interface LIF de nœud 2 IC 2 Not requise pour les configurations MetroCluster à deux nœuds (un nœud sur chaque site).
 a| 
 a| 
 a| 
 a| 
...
Exemple utilisé : Controller_B_1
 a| 
 a| 
 a| 
 a| 
...
Exemple utilisé : Controller_B_2
 a| 
 a| 
 a| 
 a| 
...
La même chose dans les deux types de clusters
 ...
 a| 
Configurer les nœuds sur le cluster en tant que paires haute disponibilité
 a| 
La même chose dans les deux types de clusters
...
| Tiroir disque (sample_shelf_name)... | Sur le site... | Appartient à... | Et est attribué à ce nœud... 
...
. Si nécessaire, affectez explicitement des disques sur les tiroirs disques connectés au pool approprié en utilisant le `disk assign` commande.
+
...
NOTE: Le pool 0 contient toujours les disques qui se trouvent sur le même site que le système de stockage qui les possède.
...
Pour obtenir des informations plus générales sur la configuration de ONTAP, reportez-vous au _Guide de configuration du logiciel_
...
. Suivez les invites pour terminer l'assistant de configuration du cluster et appuyez sur entrée pour accepter les valeurs par défaut ou saisir vos propres valeurs, puis sur entrée.
+
...
. Une fois l'assistant de configuration du cluster terminé et qu'il quitte, vérifiez que le cluster est actif et que le premier nœud fonctionne correctement :
+
`cluster show`
...
.. Activer le mode sans commutateur-cluster : `network options switchless-cluster modify -enabled true`
...
+
L'exemple suivant crée les LIFs intercluster « cluster01_icl01 » et « cluster01_icl02 » dans le groupe de basculement intercluster 01 :
...
. Vérifiez que la relation entre clusters a été créée : `cluster peer show -instance`
+
[listing]
----
...
Vous pouvez utiliser la commande cluster peer create pour initier une demande de relation de peering entre un cluster local et distant. Une fois la relation peer-to-peer demandée par le cluster local, vous pouvez exécuter cluster peer create sur le cluster distant afin d'accepter la relation.
...
* Les noms d'agrégats doivent être conformes au schéma de nommage que vous avez déterminé lors de la planification de votre configuration MetroCluster. Voir https://["Gestion des disques et des agrégats"^].
...
+
Si vous êtes connecté au cluster depuis l'interface de gestion du cluster, vous pouvez créer un agrégat sur n'importe quel nœud du cluster. Pour vous assurer que l'agrégat est créé sur un nœud spécifique, utilisez le paramètre -node ou spécifiez les disques qui appartiennent à ce nœud.
...
Le contrôle est possible grâce à la commande Storage aggry show.


NOTE: Pour utiliser un seul agrégat de données en miroir, reportez-vous à l'étape 1 de link:concept_configure_the_mcc_software_in_ontap.html["Configurez le logiciel MetroCluster dans ONTAP"] pour obtenir des instructions.
...
.. Configurer le MetroCluster avec le paramètre –allow-with-one-agagrégat true :
+
...
[NOTE]
====
Il est recommandé d'avoir plusieurs agrégats de données. Si le premier groupe de reprise après incident ne dispose que d'un seul agrégat et que vous souhaitez ajouter un groupe de reprise après incident avec un seul agrégat, vous devez déplacer le volume de métadonnées depuis cet agrégat. Pour plus d'informations sur cette procédure, voir http://["Déplacement d'un volume de métadonnées dans les configurations MetroCluster"^].
...
Vous devez configurer soit la livraison dans l'ordre (IOD), soit la livraison hors commande (OOD) des trames en fonction de la configuration du commutateur Fibre Channel (FC). Si le commutateur FC est configuré pour IOD, le logiciel ONTAP doit être configuré pour IOD. De la même façon, si le commutateur FC est configuré pour OOD, ONTAP doit être configuré pour OOD.
...
.Description de la tâche
ONTAP prend actuellement en charge le chiffrement AES-128 et AES-256.
...
+
Si vous avez besoin d'un accès RO, après ""User (ro):`", indiquez "snmpv3user" comme indiqué dans l'exemple :
...
----
+
L'exemple montre comment configurer un utilisateur en lecture seule. Vous pouvez régler les utilisateurs RW si nécessaire. Vous devez également définir des mots de passe pour les comptes inutilisés afin de les sécuriser et d'utiliser le meilleur cryptage disponible dans votre version de ONTAP.
...
.Informations associées
https://["Configuration haute disponibilité"^]
...
.Étape
. Utilisez les procédures négociées pour le basculement, la réparation et le rétablissement https://["Gestion et reprise après incident MetroCluster"]
...
+
https://["Gestion des sauvegardes de configuration"^] contient des informations supplémentaires.



.Informations associées
https://["Administration du cluster"^]
