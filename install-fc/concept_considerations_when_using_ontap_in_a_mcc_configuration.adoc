...
---
= Considérations ONTAP pour les configurations MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image::../media/svm_dr_normal_behavior.gif[Récupération après sinistre SVM pendant le fonctionnement en régime permanent]
...
+
image::../media/svm_dr_image_2.gif[Récupération après sinistre SVM après basculement]
...
Consultez la section « `Réplication de la configuration SVM` » dans le link:http://docs.netapp.com/ontap-9/topic/com.netapp.doc.pow-dap/home.html["Protection des données via l'interface de ligne de commandes"^] pour plus de détails sur la configuration d'une relation SVM DR.
...
image::../media/svm_dr_resynchronization.gif[Resynchronisation de la reprise après sinistre SVM]
