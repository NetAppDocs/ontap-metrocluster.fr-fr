...
---
= Configurez le MetroCluster logiciel FC dans ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Vous devez configurer chaque nœud dans la configuration FC MetroCluster dans ONTAP, y compris les configurations au niveau du nœud et la configuration des nœuds en deux sites. Vous devez également implémenter la relation MetroCluster entre les deux sites.

image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[Workflow pour la configuration de nœuds et de clusters de haut niveau]
...
Exemple utilisé dans cette procédure : site_B
 a| 
...
Exemple utilisé dans cette procédure : controller_B_1
 a| 
 a| 
 a| 
 a| 
...
Exemple utilisé dans cette procédure : controller_B_2
 a| 
 a| 
 a| 
 a| 
...
[[snmpv3-mcc-fc]]
=== Configuration de SNMPv3 dans une configuration MetroCluster FC

.Avant de commencer
Les protocoles d'authentification et de confidentialité sur les switches et sur le système ONTAP doivent être identiques.

.Description de la tâche
ONTAP prend actuellement en charge AES-128 pour la confidentialité (chiffrement) et SHA-256 pour l'authentification.

[CAUTION]
====
* Dans Fabric OS 10.x et versions ultérieures, Brocade prend uniquement en charge SHA-512 pour l'authentification SNMP. ONTAP ne prend actuellement pas en charge SHA-512, avec une prise en charge limitée à SHA‑256.
* Si vos commutateurs FC MetroCluster exécutent Fabric OS 10.x ou une version ultérieure, vous ne pouvez pas configurer SNMP et vous ne pouvez pas ajouter ces commutateurs à la surveillance de l'état ONTAP. Si vous souhaitez rétrograder le firmware du commutateur, vous pouvez suivre les étapes dans link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html["Mettez à niveau ou déclaradez le micrologiciel sur un commutateur FC Brocade"].
...
À partir de Fabric OS 9.0.1, SNMPv2 n'est plus pris en charge pour la surveillance de l'état sur les commutateurs Brocade, vous devez utiliser SNMPv3 à la place. Si vous utilisez SNMPv3, vous devez configurer SNMPv3 dans ONTAP avant de passer à la section suivante. Pour plus de détails, voir <<snmpv3-mcc-fc,Configuration de SNMPv3 dans une configuration MetroCluster FC>>.
