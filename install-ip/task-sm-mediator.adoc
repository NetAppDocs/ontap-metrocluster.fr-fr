...
NOTE: Vous pouvez uniquement configurer un ONTAP Mediator utilisant HTTPS via la CLI. System Manager n'est pas pris en charge. Pour plus d'informations, consultez link:concept_mediator_requirements.html#select-a-communication-protocol-for-ontap-mediator["Sélectionnez un protocole de communication pour ONTAP Mediator"].
