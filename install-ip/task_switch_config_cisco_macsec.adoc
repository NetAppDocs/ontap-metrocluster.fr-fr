...
summary: Si nécessaire, configurez le chiffrement MACsec uniquement sur les ports WAN ISL reliant les sites. Si vous optez pour le chiffrement MACsec, configurez-le après avoir appliqué le fichier RCF approprié. 
---
= Configurer le chiffrement MACsec sur les commutateurs Cisco 9336C dans un site IP MetroCluster (facultatif)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Si nécessaire, vous pouvez configurer le chiffrement MACsec sur les ports WAN ISL reliant les sites. Si vous optez pour le chiffrement MACsec, vous devez le configurer après avoir appliqué le fichier RCF approprié.

[NOTE]
====
* Le chiffrement MACsec ne peut être appliqué qu'aux ports WAN ISL qui relient les sites.
* La configuration du chiffrement MACsec est facultative lors de la configuration des commutateurs Cisco 9336C.
