...
[role="lead"]
Depuis ONTAP 9.4, les configurations MetroCluster IP prennent en charge les nouvelles installations avec les systèmes AFF à l'aide du partitionnement de disque avancé (ADP). Dans la plupart des configurations, le partitionnement et l'affectation des disques sont effectués automatiquement lors de la configuration initiale des sites MetroCluster.

Les versions ONTAP 9.4 et ultérieures incluent les modifications suivantes pour la prise en charge d'ADP :
...
ADP est exécuté automatiquement lors de la configuration initiale de la plate-forme.


NOTE: Depuis ONTAP 9.5, l'assignation automatique des disques doit être activée avec la commande `storage disk option modify -autoassign on`.
...
=== ADP et affectation des disques sur les systèmes AFF A220
...
=== Affectation des disques et ADP sur les systèmes AFF A250
...
Nombre minimal de disques pris en charge (par site)
 a| 
16 disques internes uniquement
 a| 
Les lecteurs sont répartis en quatre groupes égaux. Chaque tiroir est automatiquement attribué à un pool distinct.
 a| 
Chacun des deux plexes de l'agrégat racine inclut les partitions suivantes :
...
=== Affectation d'un disque ou d'un ADP sur les systèmes AFF A400
...
=== ADP et affectation des disques sur les systèmes AFF A800
...
Nombre minimal de disques pris en charge (par site)
 a| 
24 disques internes uniquement
 a| 
