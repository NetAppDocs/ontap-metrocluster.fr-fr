...
|===
+
** Méthode 1 :  to upgrade EFOS with downloading the software image to the backup boot partition
** Méthode 2 :  to upgrade EFOS using the ONIE OS installation
...
NOTE: Dans de rares cas, le commutateur peut ne pas démarrer. Suivez le  to upgrade EFOS using the ONIE OS installation pour installer la nouvelle image.
...
+
.. Configurer les paramètres de base du commutateur. Suivez l'étape 4 et les versions ultérieures :  the Broadcom IP switch to factory defaults
.. Créez et appliquez le fichier RCF comme indiqué dans  and installing the Broadcom RCF files
