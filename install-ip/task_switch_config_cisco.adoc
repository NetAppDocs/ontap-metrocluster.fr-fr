...
---
= Configurez les commutateurs IP Cisco pour une configuration IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
== Réinitialisez le commutateur IP Cisco sur les paramètres d'usine par défaut
...
* Si vous utilisez ces étapes dans le cadre d'une autre procédure, telle qu'un remplacement ou une mise à niveau de commutateur, vérifiez si vous devez déplacer les LIF du cluster vers le commutateur partenaire pour permettre des opérations de cluster non perturbatrices. link:task_install_and_cable_the_mcc_components.html["Découvrez comment configurer les commutateurs IP MetroCluster"].
...
== Téléchargez et installez le logiciel Cisco switch NX-OS
...
.Description de la tâche
Cette tâche nécessite un logiciel de transfert de fichiers, tel que FTP, TFTP, SFTP, HTTP ou SCP, pour copier les fichiers sur les commutateurs.

* Si vous utilisez ces étapes dans le cadre d'une autre procédure, telle qu'un remplacement ou une mise à niveau de commutateur, vérifiez si vous devez déplacer les LIF du cluster vers le commutateur partenaire pour permettre des opérations de cluster non perturbatrices. link:task_install_and_cable_the_mcc_components.html["Découvrez comment configurer les commutateurs IP MetroCluster"].
* Vous devez utiliser la version du logiciel de commutation prise en charge. Consultez le https://hwu.netapp.com["NetApp Hardware Universe"].
...
. Répétez les étapes précédentes sur les trois commutateurs IP restants dans la configuration IP MetroCluster.
+

NOTE: Vous devez mettre à jour les commutateurs dans l'ordre suivant : Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Si votre procédure nécessite de déplacer les LIF de cluster, vous devez répéter les étapes pour déplacer les LIF de cluster sur le second fabric (Switch_A_2, Switch_B_2) après avoir configuré les commutateurs du premier fabric (Switch_A_1, Switch_B_1). Reportez-vous à link:task_install_and_cable_the_mcc_components.html["Découvrez comment configurer les commutateurs IP MetroCluster"].

. Vérifiez que la restauration automatique est activée pour les LIF du cluster :
+
`network interface show -vserver Cluster -fields auto-revert`

+

CAUTION: Vous devez vous assurer que la restauration automatique est activée pour les LIF du cluster.

. Si la restauration automatique n'est pas activée, modifiez toutes les LIF du cluster pour activer la restauration automatique :
+

NOTE: Si vous effectuez ces étapes dans le cadre d'une autre procédure, telle qu'un remplacement ou une mise à niveau de commutateur, vous avez peut-être désactivé la restauration automatique dans le cadre des étapes de link:switch-move-cluster-lifs.html["Déplacez les LIF du cluster"].
...
== Téléchargez et installez les fichiers RCF IP Cisco
