...
---
= Exigences relatives aux ISL IP MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Ces exigences s'appliquent aux ISL sur toutes les configurations IP MetroCluster :
...
+
** À compter de ONTAP 9.18.1, les plateformes disposant d’un port backend IP MetroCluster de 100Gbps requièrent une vitesse de liaison ISL minimale de 100Gbps. L’utilisation d’une vitesse ISL différente nécessite une demande de Feature Policy Variance Request (FPVR). Pour soumettre une FPVR, contactez votre équipe commerciale NetApp.
+

CAUTION: Les mises à niveau vers ONTAP 9.18.1 et versions ultérieures sur les plateformes qui ne répondent pas actuellement à l'exigence ISL de 100Gbps ne sont pas bloquées et peuvent être effectuées. Cependant, NetApp recommande fortement aux clients de migrer vers une connectivité ISL de 100Gbps afin de maintenir les niveaux de performance et de disponibilité attendus.
