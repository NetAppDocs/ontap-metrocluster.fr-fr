...
* Cette tâche peut être effectuée sur l'interface ONTAP de n'importe quel nœud de la configuration IP MetroCluster.
* À partir d'ONTAP 9.18.1 et d'ONTAP Mediator 1.11, une seule instance d'ONTAP Mediator peut gérer jusqu'à dix MetroCluster configurations simultanément. Dans les versions antérieures, ONTAP Mediator peut prendre en charge jusqu'à cinq MetroCluster configurations simultanément.
...
.Utiliser IPv4
--
.. Exécutez la commande suivante pour ajouter le médiateur ONTAP :
+
`metrocluster configuration-settings mediator add -mediator-address <mediator_host_ip_address>`

+

NOTE: Il vous est demandé de saisir le nom d'utilisateur et le mot de passe du compte utilisateur administrateur du Mediator.
...
. Vous pouvez configurer en option le basculement forcé automatique de MetroCluster.
+
Vous ne pouvez utiliser la commande suivante qu'avec un niveau de privilèges avancé.
