...
* À partir d'ONTAP 9.19.1 et ONTAP Mediator 1.12, vous pouvez configurer un ONTAP Mediator qui utilise HTTPS pour la communication au lieu d'iSCSI. Lorsque ONTAP Mediator est configuré avec HTTPS, une seule configuration IP MetroCluster par instance d'ONTAP Mediator peut être surveillée. Pour plus d'informations, consultez <<Sélectionnez un protocole de communication pour ONTAP Mediator>>.
...
[CAUTION]
====
* Si vous configurez ONTAP Mediator avec HTTPS, une seule instance d'ONTAP Mediator ne peut surveiller qu'une seule configuration IP MetroCluster. La surveillance simultanée de plusieurs configurations IP MetroCluster n'est pas prise en charge.
...
====
Pour plus d'informations, consultez link:https://docs.netapp.com/us-en/ontap/mediator/index.html#https-iscsi-installation["Options d'installation HTTPS et iSCSI pour ONTAP Mediator"^].
